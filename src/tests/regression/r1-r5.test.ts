...
const PRACTICE_CFG: SessionConfig = { ...CFG, mode: 'practice' };

describe('R4 — timer integrity across refresh and expiry', () => {
  it('practice: refresh at T-10:00 restores remaining within ±1 s', async () => {
    const storage = new MemoryStorage();
    const h1 = makeHarness({ storage });
    await generateFully(h1, PRACTICE_CFG);
  ...
  it('exam: an unfinished exam found on reload is deleted, never resumed', async () => {
    const storage = new MemoryStorage();
    const h1 = makeHarness({ storage });
    await generateFully(h1); // CFG is exam mode
    h1.store.getState().start();
    const sess = h1.store.getState().session!;
    ...
    expect(h2.store.getState().session).toBeNull();
    expect(await storage.getSession(sess.id)).toBeUndefined(); // gone for good
  });

  it('abandon() deletes the live attempt from state and storage', async () => {
    const storage = new MemoryStorage();
    const h = makeHarness({ storage });
    await generateFully(h);
    h.store.getState().start();
    const sess = h.store.getState().session!;
    h.store.getState().answer(sess.questions[0].id, 'A');
    await new Promise((r) => setTimeout(r, 0));

    await h.store.getState().abandon();
    expect(h.store.getState().session).toBeNull();
    expect(await storage.getSession(sess.id)).toBeUndefined();
    expect(await storage.allAttempts()).toHaveLength(0); // never scored
  });
